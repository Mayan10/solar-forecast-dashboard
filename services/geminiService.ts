...
export const getInitialInsight = async (forecastData: ForecastDataPoint[], config: SolarPanelConfig): Promise<string> => {
    const peakPower = Math.max(...forecastData.map(d => d.power));
    const totalEnergy = forecastData.reduce((sum, d) => sum + d.power, 0);
...
2.  **Key Metrics:**
    ...
    *   **Total Energy:** The estimated total energy generated over the 24-hour period.
